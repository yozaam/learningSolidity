...
contract Auction {
    ...
    function Auction() public payable{
        ...
        // ** Start code here. 2 lines approximately. **/
        if( tokenDetails[msg.sender].remainingTokens < 1 || tokenDetails[msg.sender].remainingTokens < _count || _itemId>2){
            revert();
        }
        //** End code here. **
        
        /*Part 1 Task 5.Decrement the remainingTokens by the number of tokens bid
        Hint. "tokenDetails[msg.sender].remainingTokens" should 
        be decremented by "_count".*/
...
function getPersonDetails(uint id) public constant returns(uint,uint,address){return (bidders[id].remainingTokens,bidders[id].personId,bidders[id].addr);}}
